...
| S7 | Admin Web Core | 2 días | Separación web/móvil, Login Admin, Layout Dashboard, Migrar Dev Tools | ⚪ Pendiente |
| S8 | CRUD Productos Web | 3 días | Tabla de productos, formularios de creación/edición integrados a Firestore | ⚪ Pendiente |
| S9 | Gestión Pedidos Web | 2 días | Visor general de pedidos de clientes, estados, histórico de ventas | ⚪ Pendiente |
...
| ID | Tarea | Sprint | Estado | Notas |
|----|-------|--------|--------|-------|
| **S7-01** | Limpieza app móvil | S7 | ⚪ Pendiente | Eliminar DevTools de `perfil_screen.dart` |
| **S7-02** | Enrutamiento Web/Móvil | S7 | ⚪ Pendiente | `kIsWeb` en `main.dart` para separar UI |
| **S7-03** | Login Web Admin | S7 | ⚪ Pendiente | Pantalla de login para correo administrador |
| **S7-04** | Dashboard Layout | S7 | ⚪ Pendiente | Menú lateral (Rail/Drawer) para navegación admin |
| **S7-05** | Migrar DevTools | S7 | ⚪ Pendiente | Adaptar `configuracion_screen.dart` a la web |
| **S8-01** | Listado de Productos Web | S8 | ⚪ Pendiente | `DataTable` con datos de Firestore en tiempo real |
| **S8-02** | Crear Producto Web | S8 | ⚪ Pendiente | Formulario robusto de creación |
| **S8-03** | Editar/Eliminar Producto | S8 | ⚪ Pendiente | Modificar stock, precio, estado desde la web |
| **S9-01** | Listado de Pedidos Web | S9 | ⚪ Pendiente | Ver todos los pedidos centralizados |
| **S9-02** | Detalle y Estado | S9 | ⚪ Pendiente | Cambiar estados (preparando, enviado) desde la web |
